  ...
  return (
    <article className={`flex flex-col items-center overflow-hidden pb-12 break-words md:px-4 ${bottomBorder} ${backgroundColor}`}>
      ...
      <div className={`md:flex ${alternative ? "md:flex-row-reverse" : "md:flex-row"} items-start`}>
        ...
        <div style={{ lineHeight: "1.5em" }} className={`text-lg px-3 py-5 sm:max-w-lg sm:px-0 sm:text-xl md:text-2xl md:pb-16 md:pt-0 md:max-w-6xl md:px-12 lg:px-16 ${paragraphColor}`} dangerouslySetInnerHTML={{
          __html: /*content }} />*/
            `${formats && `<img class="mb-4 md:mb-0 md:rounded sm:max-w-lg md:max-w-xs md:float-left md:mr-8 lg:max-w-md w-full md:mt-3" src="${formats.medium.url || formats.small.url}" alt="${keywords}" />`} ${content}`
        }} />
      </div>
      {
        links &&
