...
function Article({ title, formats, keywords = "pet barber boutique perros mascota corte", content, last, alternative }) {
  ...
  return (
    <article className={`flex flex-col items-center overflow-hidden pb-12 break-words md:px-4 ${bottomBorder} ${backgroundColor}`}>
      <h2 style={{ lineHeight: "1.6em" }} className={`text-center sm:max-w-lg sm:px-0 md:text-6xl md:max-w-4xl ${titleColor} py-12 px-3 text-4xl font-bold md:py-24 ${!formats && "pb-4 md:py-12"}`}>{title}</h2>
      <div className={`md:flex ${alternative ? "md:flex-row-reverse" : "md:flex-row"} items-start`}>
        {formats && <img className="rounded sm:max-w-lg md:max-w-xs lg:max-w-md w-full mt-3" src={formats.medium.url || formats.small.url} alt={keywords} />}
